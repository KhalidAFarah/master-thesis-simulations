		...
		if (future.size() > 0) {
			...
			for(SimEntity entity : entities){
				if(entity instanceof Datacenter){
					for(Host host : ((Datacenter) entity).getHostList()){
						int usedPe = 0;
						int usedRam = 0;
						...
						msg += entity.getName() + ";" +  host.getId() + ";" + host.getNumberOfPes() + ";" + (host.getNumberOfPes()+usedPe) + ";" + host.getRamProvisioner().getRam() + ";" + (host.getRamProvisioner().getAvailableRam()+usedRam) + ";\n";
					}
				}
			}
