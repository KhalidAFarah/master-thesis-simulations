...
public class NetDatacenterBroker extends SimEntity {
	...
	protected void createVmsInDatacenterBase(int datacenterId) {
		...
		if(!vmids.isEmpty()){
			for(int i = 0; i < cloudletList.size(); i++){
				//appCloudletRecieved.put(i, cloudletList.get(i).getVmId());
				this.getCloudletSubmittedList().add(cloudletList.get(i));
				cloudletsSubmitted++;

				sendNow(
						getVmsToDatacentersMap().get(cloudletList.get(i).getVmId()),
						CloudSimTags.CLOUDLET_SUBMIT,
						cloudletList.get(i));
				System.out.println("Cloudlet " + (i) + " Submitted");
			}
	...
	private void CreateVMs(int datacenterId) {
		...
		for(int i = 0; i < getVmList().size(); i++){
			linkDC.processVmCreateNetwork(getVmList().get(i));
			getVmsToDatacentersMap().put(getVmList().get(i).getId(), datacenterId);
