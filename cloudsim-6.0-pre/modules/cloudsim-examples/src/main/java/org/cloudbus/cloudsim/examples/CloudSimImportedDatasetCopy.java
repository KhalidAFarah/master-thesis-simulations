...
public class CloudSimImportedDatasetCopy {
    private static List<Cloudlet> cloudlets;
    private static List<PowerVm> VMs;
    ...
    private static PowerDatacenter createDatacenter(String name, int hosts, List<PowerHostUtilizationHistory> list){
        // Here are the steps needed to create a PowerDatacenter:
        // 1. We need to create a list to store one or more
        //    Machines

        List<PowerHostUtilizationHistory> hostList;
        if(list != null){
            hostList = list;
        }else{
        ...
        for (int i = 0; i < hosts;i++){
            ...
            int hostId = i; //425021
            int ram = 4000*4*10;//16384*4*1000; //host memory (MB)
    ...
    public static List<PowerVm> DatasetVMPerformance(int broker_id) {
        List<PowerVm> vmFromDataset = new LinkedList<>();
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader("C:\\Users\\khali\\Documents\\Master Thesis\\cloudsim-6.0-pre\\modules\\cloudsim-examples\\src\\main\\resources\\datasets\\Bitbrains\\3.csv"))) {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                PowerVm vm = new PowerVm(vm_id, broker_id, mips*CPUCores, CPUCores, (int) (memoryCapacityProvisioned/1000), bw, size, 1, vmm, new CloudletSchedulerDynamicWorkload(mips*CPUCores, CPUCores), 300);
    ...
    public static void main(String[] args){
        try{
            ...
            PowerDatacenter datacenter1 = createDatacenter("Datacenter_1", 160, null);
            PowerDatacenter datacenter2 = createDatacenter("Datacenter_2", 160, datacenter1.getHostList());
            PowerDatacenter datacenter3 = createDatacenter("Datacenter_3", 160, datacenter2.getHostList());
            ...
            String msg = "datacenter_name;host_id;type;number_of_pes;available_pes;mips;available_mips;ram;available_ram;bw;available_bw;power_model;vms;\n";
                ...
                for(Host host : datacenter.getHostList()){
                    if(host instanceof PowerHost){
                        ...
                        for(double utilization : utilizations){
                            powermodel += ((PowerHost) host).getPowerModel().getPower(utilization) + ",";
                        }
                        String vmInfo = "";
                        for (Vm vm : host.getVmList()){
